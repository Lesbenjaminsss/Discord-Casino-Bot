...
def get_top1_role_id() -> int:
    env = load_env_file()
    raw = env.get("TOP1_ROLE_ID", os.environ.get("TOP1_ROLE_ID", "1510609851645165708"))
    return int(_clean(raw))


def get_afk_voice_channel_id() -> int | None:
    env = load_env_file()
    raw = env.get("AFK_VOICE_CHANNEL_ID", os.environ.get("AFK_VOICE_CHANNEL_ID", "1505958238670028885"))
    raw = _clean(raw)
    if not raw:
        return None
    try:
